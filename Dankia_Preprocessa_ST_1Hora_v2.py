...
interval = 60  # minutos
num_intervals = int(((max_date - min_date).total_seconds() + 1) / (
            60 * interval))  # +1 para garantir que o último intervalo inclua a última data
...
for idx, group in df.groupby(['Dispensário', 'Gaveta', 'Cód. Produto']):
    ...
    # Calcular estatísticas sobre as quantidades para cada série temporal
    stats = {
        'Dispensário': dispensary,
        'Gaveta': drawer,
        'Cód. Produto': product_code,
        'Quantidade Máxima': max(quantities),
        'Quantidade Mínima': min(quantities),
        'Quantidade Média': sum(quantities) / len(quantities),
        'Desvio Padrão': pd.Series(quantities).std()
    }

    quantity_stats.append(stats)
...
# Definir o índice como Tempo de Indexação
new_df.set_index('Tempo de Indexação', inplace=True)

# Passo 8: Salvar o DataFrame em um arquivo CSV
output_file_path = "/Users/ricardohausguembarovski/Desktop/series_temporais_1Hora.csv"
...
# Passo 10: Exibir estatísticas sobre as quantidades para cada série temporal
for stats in quantity_stats:
    print("\nEstatísticas para a série temporal:")
