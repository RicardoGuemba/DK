...
# Carregar dados
file_path = "/Users/ricardohausguembarovski/Desktop/st_1hora.csv"
...
# Previsões para os próximos 8 intervalos
...
plt.plot(demand_series.index[-1] + pd.to_timedelta(np.arange(1, 9), unit='H'), forecast, color='red', linestyle='--', label='Previsão')
...
for i, value in enumerate(forecast):
    forecast_time = demand_series.index[-1] + pd.to_timedelta(i + 1, unit='H')
    print(f"Dia {forecast_time.date()} Hora {forecast_time.hour}: {int(value)}")

# Avaliação do modelo
mape = MAPE(demand_series.values[-8:], forecast)
rmse = RMSE(demand_series.values[-8:], forecast)
mae = MAE(demand_series.values[-8:], forecast)
